        ...
        for ind, (name, layer_keys) in enumerate(layers.items()):
            ...
            if draw_checkerbox:
                ...
                if checker_mask[ind % n_cols]: # only draw the rectangle if it's the proper layer for which to do so
                    ...
                    self.out.write(
                        f'<rect transform="translate({-round(outer_pad_w)}, 0)" height="{round(392.0+outer_pad_h)}" width="{round(col_width)}" class="checker_background" style="fill: rgba(0,0,0,0.05)"></rect>\n'
                        )
